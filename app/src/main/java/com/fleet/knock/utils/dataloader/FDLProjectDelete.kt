...
import com.fleet.knock.info.mapper.ProjectDeleteMapper
import com.google.firebase.firestore.FirebaseFirestore
import kotlinx.coroutines.tasks.await

class FDLProjectDelete() : FDLoader<String>(COL_NAME, ProjectDeleteMapper()){
        ...
        try {
            ...
            if (!query.isEmpty) {
                for (dc in query.documentChanges) {
                    val projectId = mapper.mapping(dc.document) ?: continue
    ...
    companion object{
        const val COL_NAME = "projects_delete"

        const val FIELD_UPDATE_TIME = "update_time"

        const val CONFIG_RECENT_UPDATE_TIME = "Config.ProjectDelete.RecentUpdateTime"
    }
}