...
import com.fleet.knock.info.mapper.ThemeDeleteMapper
import com.google.firebase.firestore.FirebaseFirestore
import kotlinx.coroutines.tasks.await
import java.lang.Exception

class FDLThemeDelete() : FDLoader<String>(COL_NAME, ThemeDeleteMapper()){
        ...
        try {
            ...
            if(!query.isEmpty) {
                ...
                for (dc in query.documentChanges) {
                    val themeId = mapper.mapping(dc.document) ?: continue

                    themeDao.getSync(themeId)?.run {
                        deleteFile(context)
                    }
    ...
    companion object{
        const val COL_NAME = "theme_delete"

        const val FIELD_UPDATE_TIME = "update_time"

        const val CONFIG_RECENT_UPDATE_TIME = "Config.ThemeDelete.RecentUpdateTime"
    }
}