...
class RangeProgressBar : View {
    ...
    private val posMinGap
        get() = 10f / (max - min) * (pixelRangeMax - pixelRangeMin)

    val progress
        get() = progressBar.value

    val startScreenPos
        get() = thumbs[INDEX_START].screenPos

    val endScreenPos
        get() = thumbs[INDEX_END].screenPos
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when(event.action and MotionEvent.ACTION_MASK){
            ...
            MotionEvent.ACTION_CANCEL ->{
                ...
                listener?.onStopTrackingTouch(progressBar.value, thumbs[INDEX_START].value, thumbs[INDEX_END].value)
            }
            MotionEvent.ACTION_MOVE ->{
                for(i in 0 until event.pointerCount){
                    ...
                    listener?.onRangeChanged(thumbs[INDEX_START].value, thumbs[INDEX_START].screenPos, thumbs[INDEX_END].value, thumbs[INDEX_END].screenPos)
                }
            }
        }
    ...
    fun setProgress(progress:Long, interactive:Boolean = false){
        val p = if(progress < min) min
        else if(progress > max) max
        else progress

        progressBar.setBarWithValue(p)

        if(interactive)
            listener?.onProgressChanged(p, progressBar.screenPos)

        invalidate()
    }

    fun tapStartThumb(){
        moveThumb(INDEX_START, progressBar.pos - thumbs[INDEX_START].pos)

        invalidate()
    }

    fun tapEndThumb(){
        moveThumb(INDEX_END, progressBar.pos - thumbs[INDEX_END].pos)

        invalidate()
    }

    fun resetThumb(){
        thumbs[INDEX_START].setBarWithValue(min)
        thumbs[INDEX_END].setBarWithValue(max)
    ...
    inner class EventDetector{
        ...
        private fun actionPointerDown(event:MotionEvent) : Boolean{
            ...
            currentThumb[currentTouchId] = getClosetThumb(event.getX(currentTouchIndex), event.getY(currentTouchIndex))
    ...
    abstract inner class Bar {
        ...
        protected fun calculateValue(): Long {
            ...
            return ((pos / (pixelRangeMax - pixelRangeMin) * (max - min)) + min).toLong()
        }
    ...
    inner class Thumb(private val index:Int) : Bar(){
        val thumb: Bitmap = if(index == INDEX_START) BitmapFactory.decodeResource(resources, R.drawable.ic_editor_start)
        ...
        override fun calculatePos(): Float {
            ...
            return (value - min).toFloat() / (max - min) * (pixelRangeMax - pixelRangeMin)
        }
        ...
        fun onMeasure(){
            rect.top = dpToPixel(3)
            rect.bottom = viewHeight - dpToPixel(3)
    ...
    inner class Line{
        ...
        fun onMeasure(){
            topRect.top = dpToPixel(3)
            topRect.bottom = dpToPixel(7)

            bottomRect.top = viewHeight - dpToPixel(7)
            bottomRect.bottom = viewHeight - dpToPixel(3)
    ...
    inner class Background{
        private val bg = GradientDrawable().apply{
            ...
            setColor(Color.parseColor("#5D5D5D"))
            cornerRadius = dpToPixel(8).toFloat()
        }
    ...
    interface OnChangedListener{
        fun onProgressChanged(progress:Long, posX:Float){}
        fun onRangeChanged(start:Long, startPosX:Float, end:Long, endPosX:Float){}
